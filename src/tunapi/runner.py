...
class SessionLockMixin:
    ...
    async def run_with_resume_lock(
        ...
        run_fn: Callable[[str, ResumeToken | None], AsyncIterator[TunapiEvent]],
...
class MsgspecJsonlRunnerMixin:
    """Thin mixin for runners that decode JSONL via ``msgspec``.

    Swallows ``msgspec.DecodeError`` with a warning log and returns ``[]``
    so the stream keeps flowing.  All other decode errors fall through to
    the base class.
    """
    ...
    def decode_error_events(
        ...
        *,
        ...
        import msgspec  # local import — avoid hard dep for non-msgspec runners
        ...
        if isinstance(error, msgspec.DecodeError):
            ...
            return []
        return super().decode_error_events(  # type: ignore[misc]
            raw=raw,
            line=line,
            error=error,
            state=state,
